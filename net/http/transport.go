...
import (
    ...
    "log"
    "net"
    "zhaojunlike/gt-axios/net/http/httptrace"
...
// To explicitly enable HTTP/2 on a transport, use zhaojunlike/go-http-utls/net/http2
...
func (t *Transport) onceSetNextProtoDefaults() {
    ...
    // zhaojunlike/go-http-utls/net/http2 instead of the bundled copy, try to
...
func (t *Transport) roundTrip(req *Request) (*Response, error) {
    ...
    if isHTTP {
        for k, vv := range req.Header {
            if !httpguts.ValidHeaderFieldName(k) {
                return nil, fmt.Errorf("zhaojunlike/gt-axios/net/http: invalid header field name %q", k)
            }
            for _, v := range vv {
                if !httpguts.ValidHeaderFieldValue(v) {
                    return nil, fmt.Errorf("zhaojunlike/gt-axios/net/http: invalid header field value %q for key %v", v, k)
                }
            }
        }
    }
    ...
    if req.Method != "" && !validMethod(req.Method) {
        return nil, fmt.Errorf("zhaojunlike/gt-axios/net/http: invalid method %q", req.Method)
    }
    if req.URL.Host == "" {
        req.closeBody()
...
func (e transportReadFromServerError) Error() string {
    return fmt.Sprintf("zhaojunlike/gt-axios/net/http: Transport failed to read from server: %v", e.err)
}
...
func (w *wantConn) tryDeliver(pc *persistConn, err error) bool {
    ...
    w.err = err
    if w.pc == nil && w.err == nil {
        panic("zhaojunlike/gt-axios/net/http: internal error: misuse of tryDeliver")
    }
    close(w.ready)
    return true
}
...
func (t *Transport) decConnsPerHost(key connectMethodKey) {
    ...
    if n == 0 {
        ...
        panic("zhaojunlike/gt-axios/net/http: internal error: connCount underflow")
    }
...
func (pconn *persistConn) addTLS(name string, trace *httptrace.ClientTrace) error {
    ...
    if cfg.ClientHelloID == nil {
        cfg.ClientHelloID = &tls.HelloGolang
    }
    tlsConn := tls.UClient(plainConn, cfg, *cfg.ClientHelloID)
    if cfg.ClientSessionState != nil {
        err := tlsConn.SetSessionState(cfg.ClientSessionState)
        if err != nil {

        }
    }
    //tlsConn.SetSessionCache(cfg.ClientSessionCache)
    ...
    case cm.targetScheme == "http":
        ...
        if pa := cm.proxyAuth(); pa != "" {
            pconn.mutateHeaderFunc = func(h Header) {
                h.Set("Proxy-Authorization", pa) //设置Ua
            }
        }
...
func (pc *persistConn) mapRoundTripError(req *transportRequest, startBytesWritten int64, err error) error {
    ...
    if pc.isBroken() {
        ...
        return fmt.Errorf("zhaojunlike/gt-axios/net/http: HTTP/1.x transport connection broken: %v", err)
    }
    return err
}
...
func (pc *persistConn) readLoop() {
    ...
    for alive {
        ...
        if err != nil {
            if pc.readLimit <= 0 {
                err = fmt.Errorf("zhaojunlike/gt-axios/net/http: server response headers exceeded %d bytes; aborted", pc.maxHeaderResponseSize())
            }
...
var errTimeout error = &httpError{err: "zhaojunlike/gt-axios/net/http: timeout awaiting response headers", timeout: true}
...
func (tlsHandshakeTimeoutError) Error() string {
    return "zhaojunlike/gt-axios/net/http: TLS handshake timeout"
}
